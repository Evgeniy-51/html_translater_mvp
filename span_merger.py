...
class SpanMerger:
    ...
    def should_merge_spans(current_line, next_line):
        ...
        sentence_endings = ['.', '!', '?', '...', ':', ';']
        current_ends_with_sentence = any(current_text.endswith(ending) for ending in sentence_endings)
    ...
    def merge_spans(current_line, next_line):
        ...
        last_content = re.search(r'<span[^>]*>(.*?)</span>', last_current_span, re.DOTALL)
        first_content = re.search(r'<span[^>]*>(.*?)</span>', first_next_span, re.DOTALL)
        
        if not last_content or not first_content:
            return current_line, next_line
        
        # Объединяем содержимое
        merged_content = last_content.group(1).strip() + ' ' + first_content.group(1).strip()
        ...
        merged_current = re.sub(r'<span[^>]*>.*?</span>(?=[^<]*$)', new_span, current_line, flags=re.DOTALL)
        
        # Удаляем первый span из следующей строки
        merged_next = re.sub(r'<span[^>]*>.*?</span>', '', next_line, count=1, flags=re.DOTALL)
    ...
    def process_lines(lines):
        ...
        while i < len(lines):
            ...
            if i + 1 < len(lines) and SpanMerger.should_merge_spans(current_line, lines[i + 1]):
                # Объединяем текущую и следующую строку
                merged_current, merged_next = SpanMerger.merge_spans(current_line, lines[i + 1])
